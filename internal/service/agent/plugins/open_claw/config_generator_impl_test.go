...
func TestOpenClawGenerateConfigCopiesAssets(t *testing.T) {
	root := t.TempDir()
	skillID := uuid.New()
	...
	generator := NewOpenClawConfigGenerator(skillStorage, subagentStorage, commandStorage, ruleStorage, zap.NewNop())
...
func TestOpenClawGenerateConfigFallbackAndPreview(t *testing.T) {
	root := t.TempDir()
	generator := NewOpenClawConfigGenerator(
...
func TestOpenClawCopyHelpers(t *testing.T) {
	root := t.TempDir()
	generator := NewOpenClawConfigGenerator("", "", "", "", zap.NewNop()).(*OpenClawConfigGenerator)
