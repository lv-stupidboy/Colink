...
const CommandList: React.FC = () => {
  ...
  // 查看引用的角色
  const handleViewRefs = async (command: Command) => {
    try {
      const result: AssetAgentsResponse = await api.commands.getBoundAgents(command.id);
      if (result.agents && result.agents.length > 0) {
        Modal.info({
          ...
          content: (
            <div>
              <p>该 Command 被以下 <strong>{result.count}</strong> 个角色引用：</p>
        ...
        Modal.info({
          title: '角色引用',
          content: <p>该 Command 暂未被任何角色引用</p>,
        });
      }
    } catch (error) {
