...
func (r *AgentConfigRepository) FindByID(ctx context.Context, id uuid.UUID) (*model.AgentRoleConfig, error) {
	query := `
		SELECT id, name, role, description, system_prompt, max_tokens, temperature, routing_config, base_agent_id, is_default, is_system, capabilities, dependencies, outputs, created_at, updated_at
		FROM agent_configs WHERE id = ?
	`
	config := &model.AgentRoleConfig{}
	var idStr string
	var routingConfig, capabilities, dependencies, outputs []byte
	var baseAgentID, description, systemPrompt sql.NullString
	var maxTokens sql.NullInt64
	var temperature sql.NullFloat64
	err := r.db.QueryRowContext(ctx, query, id.String()).Scan(
		&idStr, &config.Name, &config.Role, &description, &systemPrompt, &maxTokens, &temperature, &routingConfig, &baseAgentID, &config.IsDefault, &config.IsSystem, &capabilities, &dependencies, &outputs, &config.CreatedAt, &config.UpdatedAt,
	)
	if err != nil {
