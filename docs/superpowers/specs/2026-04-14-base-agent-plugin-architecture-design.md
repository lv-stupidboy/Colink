...
4. **类型混乱**：存在 OpenCode CLI（废弃）和 OpenCode(ACP) 两种类型，需清理CLI版本并去掉ACP后缀
...
| 文件/内容 | 原因 |
|-----------|------|
| `open_code_adapter.go` | CLI版本逻辑删除 |
| `BaseAgentTypeOpenCode` CLI常量 | CLI版本废弃 |
...
| 原内容 | 新内容 | 说明 |
|--------|--------|------|
| `BaseAgentTypeOpenCodeACP` | `BaseAgentTypeOpenCode` | 去掉 acp 后缀 |
| `OpenCodeACPAdapter` | `OpenCodeAdapter` | CLI版本删除，ACP版本保留并重命名 |
| 显示名称 `"OpenCode (ACP)"` | `"OpenCode"` | 不对用户暴露协议细节 |
...
6. OpenCode CLI 版本删除，ACP 版本保留并重命名为 `open_code`（去掉 acp 后缀）